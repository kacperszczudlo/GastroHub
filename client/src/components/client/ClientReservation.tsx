...
type TimeDropdownProps = { name: string; options: string[] };

function TimeDropdown({ name, options }: TimeDropdownProps) {
  ...
  return (
    <div ref={ref} className="w-full">
      ...
      {open && (
        <div
          ref={listRef}
          className="absolute left-0 right-0 mt-2 bg-white border rounded-lg shadow-lg z-50 max-h-48 overflow-y-auto"
        >
          {options.map((t) => (
            <button
              key={t}
...
export function ClientReservation() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const created = await reservationService.create(draft);
      setReservations((prev) => [...prev, created]);
      (e.currentTarget as HTMLFormElement).reset();
      showSuccess('Rezerwacja została wysłana do zatwierdzenia. Poczekaj na decyzję obsługi.');
    } catch (error) {
      const axiosError = error as {
        response?: { status?: number; data?: { details?: string; error?: string } };
      };
      try {
        ...
        const exists = mine.some(
          (item) =>
            normalizeTime(item.time) === normalizeTime(draft.time) &&
            item.guests === draft.guests &&
      ...
      const apiMessage = axiosError.response?.data?.details || axiosError.response?.data?.error;
    ...
    <div className="max-w-2xl mx-auto p-6 mt-10">
      <div className="bg-white p-8 rounded-2xl shadow-xl">
        ...
        <p className="text-gray-600 mb-6 bg-blue-50 p-4 rounded-lg border border-blue-100 text-sm">
          Wybierz termin. Po wysłaniu formularza, nasza obsługa zweryfikuje dostępność i potwierdzi rezerwację przypisując
          stolik.
        </p>
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="relative">
              ...
              <TimeDropdown name="time" options={reservationTimeSlots} />
            </div>
          </div>
          <div>
